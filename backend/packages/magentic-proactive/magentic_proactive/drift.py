...
class DriftEngine:
    ...
    async def execute(self) -> dict:
        ...
        prompt = f"""你是后台任务执行代理。在用户不活跃时自动执行维护任务。
...
## 执行规则
1. 严格按步骤说明逐个执行，不要跳过，不要添加说明中没有的步骤
2. 每完成一步，用一行简短摘要标注完成状态
3. 如果某步依赖不可用（工具缺失、文件不存在），记录原因并跳过到下一步
4. 全部完成后，输出三行总结：
   - 完成: <完成的步骤数>/<总步骤数>
   - 发现: <主要发现>
   - 建议: <下一步建议>
5. 最多 {self._max_steps} 步；若步数用完，在总结中说明未完成部分

## 约束
- 不要和用户交互，不要提问，不要等待确认——你是后台任务，用户不在线
- 不要生成超过 500 字的详细报告，保持精简
- 输出格式：每步一行 "[step N] 摘要"，最后三行总结
